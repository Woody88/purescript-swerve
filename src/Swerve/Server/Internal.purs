...
-- instance evalServerBasicAuth  :: EvalServer s (s (BasicAuth realm usr :> api) m) (usr -> (s api m))
-- instance evalServerAlt        :: EvalServer s (s (a :<|> b) m) ((s a m) :<|> (s b m))
-- instance evalServerRaw        :: TypeEquals Application application => EvalServer s (s Raw m) (m application)
-- instance evalServerRaise      :: EvalServer s (s ((Raise status hdrs ctypes) :> api) m) (s api m)
-- instance evalServerVerb       :: EvalServer s (s (Verb method a status hdrs ctypes) m) (m (Response rs a))
-- instance evalServerReqBody    :: EvalServer s (s (ReqBody a ctypes :> api) m) (a -> s api m)
-- instance evalServerCapture    :: EvalServer s (s (Capture a :> api) m) (a -> s api m)
-- instance evalServerQueryParam :: IsSymbol sym => EvalServer s (s (QueryParam sym a :> api) m) (Maybe a -> s api m)
-- instance evalServerHeader     :: IsSymbol sym => EvalServer s (s (Header sym a :> api) m) (a -> s api m)
-- instance evalServerSeg        ::  EvalServer s (s api m) b

class HasServer :: (Type -> (Type -> Type) -> Type) -> Type -> Row Type -> (Type -> Type) -> Type -> Constraint
class HasServer server api context m handler | server api m -> handler context where 
  route :: forall env. Proxy server -> Proxy api -> Proxy m -> Record context -> Delayed env (server api m) -> Router env
  hoistServerWithContext :: forall n. Proxy server -> Proxy api -> Proxy context -> (forall x. m x -> n x) -> server api m -> server api n
...
instance hasServerVerb :: 
  ...
  , ToMethod method
  , AllMime ctypes
  , EvalServer server (server (Verb method a status hdrs ctypes) m) (m (Response rs a))
  ) => HasServer server (Verb method a status hdrs ctypes) context m (m (Response (Either (Respond' status hdrs) l) a)) where 
  hoistServerWithContext st _ _ nt s = unsafeCoerce (nt $ toHoistServer st s)
  route st _ _ _ subserver = leafRouter route'
    where 
      ...
      route' env request respond = do 
        let 
          ...
          action = (toHandler st subserver) `addMethodCheck` methodCheck method request
                                            `addAcceptCheck` acceptCheck ctypesP accH
        runAction (unsafeCoerce action) env request respond 
...
instance hasServerSegment :: 
  ( IsSymbol path
  , HasServer server api context m handler
  , EvalServer server (server (path :> api) m) (server api m)
  ) => HasServer server (path :> api) context m handler where 
  hoistServerWithContext st _ pc nt s = unsafeCoerce $ hoistServerWithContext st (Proxy :: _ api) pc nt (toHoistServer st s)
  route st _ m ctx subserver = pathRouter path (route st (Proxy :: _ api) m ctx (toHandler st subserver))
    where 
